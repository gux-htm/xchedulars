...
const selectSlots = async (req, res) => {
    ...
    try {
        ...
        if (time_slots && time_slots.length > 0) {
            // Optimization: Fetch all rooms and occupied slots in bulk
            const [allRooms] = await connection.query('SELECT id FROM rooms ORDER BY capacity DESC');

            if (allRooms.length === 0) {
                await connection.rollback();
                return res.status(409).json({ error: 'No rooms defined in the system' });
            }

            const [occupiedData] = await connection.query(
                'SELECT room_id, time_slot_id FROM room_assignments WHERE time_slot_id IN (?)',
                [time_slots]
            );

            // Map time_slot_id -> Set of occupied room_ids
            const occupiedMap = new Map();
            for (const occ of occupiedData) {
                if (!occupiedMap.has(occ.time_slot_id)) {
                    occupiedMap.set(occ.time_slot_id, new Set());
                }
                occupiedMap.get(occ.time_slot_id).add(occ.room_id);
            }
            ...
            for (const time_slot_id of time_slots) {
                const occupiedRooms = occupiedMap.get(time_slot_id) || new Set();

                // Find the first room (largest capacity) that is not occupied
                const room = allRooms.find(r => !occupiedRooms.has(r.id));
                ...
                // Add to local occupied set to handle duplicate slots in request if any
                if (!occupiedMap.has(time_slot_id)) {
                    occupiedMap.set(time_slot_id, new Set());
                }
                occupiedMap.get(time_slot_id).add(room.id);
            ...
            // assignResult.insertId is the ID of the first inserted row.
            // Assuming sequential IDs for bulk inserts (standard InnoDB behavior).
            const startId = assignResult.insertId;
            const reservationValues = reservations.map((res, index) => [
                res.course_request_id,
                res.instructor_id,
                res.time_slot_id,
                startId + index,
                res.offering_id
